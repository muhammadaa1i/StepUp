/**
 * Payment Service
 * Handles OCTO payment integration
...
export class PaymentService {
  // OCTO Payment endpoints - using modular endpoint configuration
  ...
  async createPayment(paymentData: PaymentRequest, timeout = 4000): Promise<PaymentResponse> {
    try {
      ...
      const errorMsg = data.errMessage || 
        `Payment failed: Success=${data.success}, URL=${!!(
          data.octo_pay_url || data.payment_url
        )}`;
